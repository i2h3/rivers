...
A typical session creates a journal, begins one or more root activities, emits messages at `debug`, `info`, or `error` level, and calls ``Journaling/finish()`` before exit to drain pending writes.
